...
import knexConfig from '../../knexfile.mjs'; // Use the ES module knexfile
import path from 'path';
  ...
  async getMigrationStatus() {
    ...
    try {
      const [completed, pending] = await Promise.all([
        database.migrate.currentVersion(),
        database.migrate.list()
      ]);
      return {
        currentVersion: completed,
        pendingMigrations: pending[1], // pending migrations
        completedMigrations: pending[0] // completed migrations
      };
    } catch (error) {
