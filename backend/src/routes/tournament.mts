  ...
  fastify.get(
    ...
    async (request, reply: FastifyReply) => {
      try {
        ...
        return reply.status(500).send({ error: 'Failed to fetch tournament details' });
      }
    }
  );
    ...
    async (request, reply: FastifyReply) => {
      try {
        ...
        if (existingRegistration) {
          return reply.status(400).send({ error: 'You are already registered for this tournament' });
        }
    ...
    async (request, reply: FastifyReply) => {
      try {
        ...
        if (match.player1_id !== userId && match.player2_id !== userId) {
          return reply.status(403).send({ error: 'Only match participants can update scores' });
        }
        
        // Check if match is already completed
        if (match.status === 'completed') {
          return reply.status(400).send({ error: 'Cannot update score for completed match' });
        }
